...
class CollieTrainer(Seq2SeqTrainer):
    ...
    def __init__(
        ...
        # callbacks: callbacks表示一个callback函数的列表, callback函数能够在训练的特定时期自动调用
        ...
        # 修改TrainingArguments.__setattr__函数
        # 在huggingface的transformer库中, TrainingArguments类用于存储所有配置的参数
        # 但是TrainingArguments类重写了__setattr__方法, 使得TrainingArguments类在创建实例后其属性不可变
        # 因此, 这里将TrainingArguments的__setattr__方法修改为python基类object的__setattr__方法, 使其可变
        # 为了在后面动态的修改类似于学习率的这种参数
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.first_train_batch:
            ...
            if self.tokenizer is not None:
                # 将print_input_ids中值为-100的token (之前设置的) 替换为self.tokenizer.pad_token.id
        ...
        if model_name in MODEL_FOR_CAUSAL_LM_MAPPING_NAMES.values() or model_name == "PeftModelForCausalLM":
            # .contiguous()方法使得调整后的张量在内存中连续
            logits = logits[..., :-1, :].contiguous()
            labels = labels[..., 1:].contiguous()
            loss_weight_mask = loss_weight_mask[..., 1:].contiguous()

        # 展平logits [batch_size, num_words, len_hidden_state] --> [batch_size * num_words, len_hidden_state]
        logits = logits.view(-1, logits.size(-1))
        # 展平labels [batch_size, num_words] --> [batch_size * num_words]
        labels = labels.view(-1)
        loss_weight_mask = loss_weight_mask.view(-1)
        # ignore_index=-100: 忽略 label = -100 的项
...
class RotateDatasetCallback(TrainerCallback):
    ...
    def on_epoch_end(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        """
        on_epoch_end: 表示在每个epoch结束时自动调用, 由于RotateDatasetCallback类继承于TrainCallback, 因此这里不是定义而是重写
        """
