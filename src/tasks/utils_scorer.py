...
class RelationScorer(SpanScorer):
    ...
    @override
    def __call__(self, reference: List[Relation], predictions: List[Relation], **kwargs: dict) -> Dict[str, Dict[str, float]]:
...
class EventScorer(Scorer):
    ...
    @override
    def __call__(self, reference: Any, predictions: Any, **kwargs: dict) -> Dict[str, Dict[str, float]]:
        if not len(reference) or (len(reference) and not isinstance(reference[0], list)):
            reference = [reference]
        if not len(predictions) or (len(predictions) and not isinstance(predictions[0], list)):
            predictions = [predictions]
