...
def get_fabner_hf(
    ...
    """
    Get the fabner dataset from the huggingface datasets library
    Args:
    ...
    # dataset = load_dataset("DFKI-SLT/fabner", name="fabner_bio")
    dataset = load_dataset("./download/fabner")
...
class FabNerDatasetLoader(DatasetLoader):
    """
    A `DatasetLoader` for the FabNER dataset.
    ...
    def __init__(self, path_or_split: str, **kwargs) -> None:
        self.ENTITY_TO_CLASS_MAPPING = {
            "MATE": Material,
            "MANP": ManufacturingProcess,
            "MACEQ": MachineEquipment,
            "APPL": Application,
            "FEAT": EngineeringFeatures,
            "PRO": MechanicalProperties,
            "CHAR": ProcessCharacterization,
            "PARA": ProcessParameters,
        ...
        dataset_words, dataset_entities = get_fabner_hf(
            split=path_or_split,
...
class FabNerSampler(Sampler):
    """
    A data `Sampler` for the FabNER dataset.

    Args:
        dataset_loader (`FabNerDatasetLoader`):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert task in [
            "NER",
        ], f"FabNER only supports NER task. {task} is not supported."
