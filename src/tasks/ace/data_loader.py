    ...
    def __init__(self, path: str, group_by: str = "sentence", **kwargs) -> None:
        ...
        with open(path, "rt") as in_f:
            for line in in_f:
                ...
                # 获取一个数据样本的 `sent_id` 后者 `doc_id`, 作为key
                key = line["sent_id"] if group_by == "sentence" else line["doc_id"]

                # 将当前样本的key添加到self.eleents中
                ...
                # 构造存在于self.ENTITY_TO_CLASS_MAPPING中的entity_mentions
                entities = [
                ...
                # 构造存在于self.VALUE_TO_CLASS_MAPPING中的entity_mentions
                values = [
                ...
                # 构造一个instance, 并将其添加到self.elements中
...
if __name__ == '__main__':
    dataset = ACEDatasetLoader("./download/ace2005/ace05/train.sentence.json")
    print(dataset[0])