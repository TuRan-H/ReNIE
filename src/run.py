...
def train_collie(
	...
	logging.info(
		"Training dataset will be loaded with. 'ignore_pad_token_for_loss':"        # *** ignore_pad_token_for_loss: 作用是告诉模型在计算loss的过程中, 忽略pad (填充token) token
...
def inference_collie(
	...
	# training_args.predict_with_generate: Whether to use generate to calculate generative metrics (ROUGE, BLEU)
	if not training_args.predict_with_generate:
		...
		# training_args.prediction_loss_only: 在predict过程中只计算loss
	...
	# 如果在train结束后进行inference, 则加载 `training_args.output_dir` 中的模型
	if training_args.do_train:
		if not checkpoint_path:
			...
			# 如果使用了LORA, 则设置model_path为模型的地址, lora_weights_name_or_path为输出地址
			...
			if model_args.use_lora:		
				...
				lora_weights_name_or_path = None
	# 如果没有指定checkpoint_path, 则使用 `model_args.model_name_or_path` 中的模型, 以及 model_args.lora_weights_name_or_path 中的lora weight
	else:
		if not checkpoint_path:
			model_path = model_args.model_name_or_path
	...
	# 避免模型在生成过程中出现Setting pad_token_id to eos_token_id:2 for open-end generation
	...
	for test_task in data_args.test_tasks:
		# 将data_args.test_tasks中的元素合称为一个真正的文件地址
		...
		if training_args.predict_with_generate:
			...
			with open(output_name, "w", encoding="utf8") as f:
				...
				# 将之前设置为 -100 的token全部设为 `tokenizer.pad_token_id`
...
if __name__ == "__main__":
	...
	if training_args.do_predict and data_args.test_tasks is not None:
		# 如果不评估所有的checkpoints, 则使用 `training_args.output_dir` 中的模型
