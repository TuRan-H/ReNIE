...
) -> (str, Union[int, List[int]]): # type: ignore
    """
...
def load_model(
    ...
    tokenizer: PreTrainedTokenizerBase = AutoTokenizer.from_pretrained(
        model_weights_name_or_path,
        add_eos_token=True,     # 每个样本tokenizer结束后, 都会在其最后面添加一个eos_token
        trust_remote_code=trust_remote_code,
    )
    ...
    #  Load the model weights
    # ************************************************** 部分transformers版本中吗load_in_4_bit和quantization_config不可同时存在
    ...
    # quant_args['load_in_4bit'] = False if 'load_in_4bit' in quant_args else None
    # model: PreTrainedModel = load_fn.from_pretrained(
    #     pretrained_model_name_or_path=model_weights_name_or_path,
    #     device_map=device_map,
    #     max_memory=max_memory,
    ...
    #     **quant_args,
    # )
    # quant_args['load_in_4bit'] = True if 'load_in_4bit' in quant_args else None
    # ************************************************** 部分transformers版本中吗load_in_4_bit和quantization_config不可同时存在
    ...
    if use_lora:
        ...
        if lora_weights_name_or_path is None:
            ...
            lora_config = LoraConfig(
                r=lora_r,                   # r: 低秩矩阵的维度
                lora_alpha=lora_alpha,      # lora_alpha: 低秩矩阵的缩放因子
                lora_dropout=lora_dropout,  # lora_dropout: 低秩矩阵的dropout概率
                bias="none",
                task_type=TaskType.CAUSAL_LM if model_type == "causal" else TaskType.SEQ_2_SEQ_LM,
                target_modules=lora_target_modules,
            )
            # *** 使用get_peft_mode将model和lora_config包装为PeftModel, PeftModel可以实现LoRA
    ...
    if inference:
        if use_lora:
            if quantization is None:
                ...
                #  * 如果是inference阶段, 会在inference之前将lora合并到模型中
