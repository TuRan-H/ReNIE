...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(LLAMA_INPUTS_DOCSTRING)      # *** 装饰器add_start_docstring_to_model_forward()
    ...
    def forward(
        ...
        outputs = self.model(
            ...
            use_cache=use_cache,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,          # output_hidden_states: 表示是否返回每一层的hidden state, 正常来说只需要最后一层的hidden state就行了
            return_dict=return_dict,                            # return_dict表示是否以字典形式返回
            is_padded_inputs=is_padded_inputs,                  # is_padded_inputs表示输入是否被padding (假设attention存在, 且存在某一个项为0)
        )

        hidden_states = outputs[0]              # outputs是模型的输出, outputs[0] == outputs.last_hidden_state
        if self.config.pretraining_tp > 1:      # pretraining_tp: 表示并行计算
            lm_head_slices = self.lm_head.weight.split(self.vocab_size // self.config.pretraining_tp, dim=0)
            logits = [F.linear(hidden_states, lm_head_slices[i]) for i in range(self.config.pretraining_tp)]
            logits = torch.cat(logits, dim=-1)
        else:
            """
            * lm_head代指语言模型的最后一层, 最后一层一般是一个fully connect层
            * hidden_state的shape为 [seq_length, hidden_size], 在fully connect层中与一个矩阵 [hidden_size, vocab_size]相乘, 生成[seq_length, vocab_size]的矩阵
            * 这个矩阵每一行表示根据某个word x预测出词汇表中另一个word y的未归一化的概率
            """
